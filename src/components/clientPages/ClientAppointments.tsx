...
import { PROCEDURE_OPTIONS } from "../models/Info";
...
const SearchAndHistory = ({
    ...
    return (
        <div className="w-full">
            ...
            <h2 className="text-xl font-bold mb-4 text-[#303030]">
                Συμπληρώστε όσες πληροφορίες επιθυμείτε
            </h2>
            ...
            <div className="w-full shadow-md rounded-lg overflow-hidden mb-4 border border-gray-300">
                <div className="grid grid-cols-[1fr_1.5fr_160px]">
                    ...
                    <div className="bg-white flex items-center justify-center px-2 border-r border-gray-200 h-16">
                        <input
                            type="date"
                            className="w-full text-sm text-gray-600 focus:outline-none bg-transparent text-center cursor-pointer"
                            value={searchFilters.date}
                            onChange={(e) =>
                                setSearchFilters((prev: any) => ({ ...prev, date: e.target.value }))
                            }
                        />
                    </div>
            ...
            <div className="text-center mb-12">
                <button
                    onClick={() => {
                        setSearchFilters({ date: "", location: "" });
                        setStep("vet-list");
                    }}
                    className="text-gray-500 text-sm underline hover:text-black transition-colors cursor-pointer"
                >
                        ...
                        <tbody className="text-sm">
                            {appointments.length === 0 ? (
                                ...
                                appointments.map((item: any) => {
                                    ...
                                    const timeStr = dateObj.toLocaleTimeString("el-GR", {
                                        hour: "2-digit",
                                        minute: "2-digit",
                                    });
                                    ...
                                    if (item.status === "new" || item.status === "pending") {
                                        statusLabel = "Εκκρεμές";
                                        statusClass = "bg-yellow-100 text-yellow-800";
                                    }
                                    if (item.status === "approved") {
                                        statusLabel = "Εγκεκριμένο";
                                        statusClass = "bg-blue-100 text-blue-800";
                                    }
                                    if (item.status === "completed") {
                                        statusLabel = "Πραγματοποιημένο";
                                        statusClass = "bg-green-100 text-green-800";
                                    }
                                    if (item.status === "canceled") {
                                        statusLabel = "Ακυρωμένο";
                                        statusClass = "bg-red-100 text-red-800";
                                    }

                                    const petName =
                                        item.petName ||
                                        pets.find((p: any) => p.id === item.petId)?.name ||
                                        "-";
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className="py-4 pl-4 rounded-l-xl font-medium text-gray-700">
                                                {PROCEDURE_OPTIONS[item.reason] ?? item.reason}
                                            </td>
                                                ...
                                                <div className="flex justify-end gap-2">
                                                    {(item.status === "new" ||
                                                        ...
                                                        <button
                                                            ...
                                                            className="p-2 bg-yellow-100 rounded-lg hover:bg-yellow-200 text-yellow-700 transition-colors cursor-pointer"
                                                            title="Επεξεργασία"
                                                        >
                                                            <svg
                                                    ...
                                                    {(item.status === "new" ||
                                                        ...
                                                        <button
                                                            ...
                                                            className="p-2 bg-[#fcdcdc] rounded-lg hover:bg-[#f5c6c6] text-red-600 transition-colors cursor-pointer"
                                                            title="Ακύρωση"
                                                        >
                                                            <svg
                                                                xmlns="http://www.w3.org/2000/svg"
...
const VetList = ({ vets, searchFilters, setSearchFilters, setStep, setSelectedVet }: any) => {
    const filteredVets = vets.filter((vet: any) => {
        const matchesLocation =
            ...
            (vet.address &&
                vet.address.toLowerCase().includes(searchFilters.location.toLowerCase()));
                        ...
                        >
                            ...
                            <div className="p-6 flex flex-col justify-between flex-1">
                                ...
                                <button
                                    ...
                                    className="mt-4 w-full py-2.5 border border-gray-300 rounded-xl hover:bg-gray-50 font-medium text-gray-700 transition-colors cursor-pointer"
                                >
                                    Προβολή
                                </button>
                            </div>
        ...
        <div className="w-full">
            ...
            <div className="flex flex-col md:flex-row gap-10 items-start mb-12">
                <div className="w-full md:w-1/3 rounded-[2rem] overflow-hidden shadow-lg h-[400px]">
                    <img
                        src={selectedVet.photo || vetProfileImg}
                        className="w-full h-full object-cover"
                        alt="Dr"
                    />
                </div>
                <div className="flex-1 pt-4">
                    <h1 className="text-4xl font-bold text-[#303030] mb-2">
                        {selectedVet.fullName || selectedVet.name}
                    </h1>
                    <p className="text-xl text-gray-500 mb-8 font-medium">
                        {selectedVet.specialty || "Κτηνίατρος"}
                    </p>
                    ...
                    <div className="mb-10 text-gray-600 space-y-2 bg-white p-4 rounded-xl w-fit shadow-sm border border-gray-200">
                        <p className="flex items-center gap-2">
                            📍{" "}
                            <span className="font-medium">
                                {selectedVet.address}, {selectedVet.city}
                            </span>
                        </p>
                        <p className="flex items-center gap-2">
                            📞 <span className="font-medium">{selectedVet.phone_number}</span>
                        </p>
                    </div>

                    <button
                        onClick={() => setStep("booking")}
                        className="bg-[#5c5c5c] text-white px-8 py-4 rounded-xl font-bold hover:bg-[#4a4a4a] shadow-lg hover:shadow-xl transition-all cursor-pointer"
                    >
                        Κλείστε Ραντεβού
                    </button>
                </div>
...
const BookingForm = ({
    ...
    return (
        <div className="w-full">
            <Stepper current={2} />
            <div className="bg-white rounded-3xl p-8 shadow-lg max-w-4xl mx-auto border border-gray-200">
                <h2 className="text-3xl font-bold mb-2 text-[#303030]">Κράτηση Ραντεβού</h2>
                <p className="text-sm text-gray-500 mb-8 font-medium">
                    με τον/την {selectedVet?.fullName || selectedVet?.name}
                </p>

                {/* Pet Selection */}
                <div className="mb-6">
                    <label className="text-xs font-bold ml-1 mb-1 block text-gray-600">
                        Επιλογή Κατοικιδίου
                    </label>
                    <select
                        className="w-full bg-white border border-gray-300 p-3.5 rounded-xl text-sm font-medium cursor-pointer"
                        onChange={(e) =>
                            setBookingData((prev: any) => ({ ...prev, petId: e.target.value }))
                        }
                ...
                <div className="mb-6">
                    <label className="text-xs font-bold ml-1 mb-1 block text-gray-600">
                        Αιτιολογία / Τύπος Ραντεβού
                    </label>
                    <select
                        className="w-full bg-white border border-gray-300 p-3.5 rounded-xl text-sm font-medium cursor-pointer"
                        onChange={(e) =>
                            setBookingData((prev: any) => ({ ...prev, reason: e.target.value }))
                        }
                        value={bookingData.reason}
                    >
                        <option value="">Εμφάνιση επιλογών --</option>
                        {Object.entries(PROCEDURE_OPTIONS).map(([key, label]) => (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    <div>
                        <label className="text-xs font-bold ml-1 mb-2 block text-gray-600">
                            Επιλογή Ημερομηνίας
                        </label>
                        <div className="flex flex-col items-center bg-white p-4 rounded-xl border border-gray-300">
                            <div className="flex gap-4 mb-2 text-xs text-gray-500">
                                <span className="flex items-center gap-1">
                                    <img src={Empty} className="w-3" /> Πολλή
                                </span>
                                <span className="flex items-center gap-1">
                                    <img src={Half} className="w-3" /> Μέτρια
                                </span>
                                <span className="flex items-center gap-1">
                                    <img src={Full} className="w-3" /> Καμία
                                </span>
                            </div>
                            <Calendar
                                onChange={(d) => {
                                    setDate(d as Date);
                                    setBookingData((prev: any) => ({ ...prev, date: d as Date }));
                                }}
                                value={date}
                ...
                <div className="flex justify-center">
                    <button
                        ...
                        className="bg-[#5c5c5c] text-white px-16 py-4 rounded-xl font-bold hover:bg-[#4a4a4a] text-lg shadow-lg hover:shadow-xl transition-all cursor-pointer"
                    >
                        Οριστικοποίηση
                    </button>
                </div>
...
const SuccessPage = ({ setStep }: any) => (
    <div className="w-full text-center">
        ...
        <button
            ...
            className="bg-[#5c5c5c] text-white px-8 py-3 rounded-xl font-bold hover:bg-[#4a4a4a] shadow-md cursor-pointer"
        >
            Επιστροφή
        </button>
    </div>
...
const CancelConfirmation = ({ selectedAppointment, handleCancelAppointment, setStep }: any) => (
    <div className="w-full text-center mt-8">
        <div className="bg-white rounded-4xl p-10 shadow-lg max-w-2xl mx-auto relative overflow-hidden border-t-8 border-red-500 border border-gray-200">
            ...
            <div className="bg-[#f9f9f9] rounded-xl p-6 mb-8 text-left shadow-sm border border-gray-200">
                <p>
                    <strong>Λόγος:</strong> {selectedAppointment?.reason}
                </p>
                <p>
                    <strong>Ημερομηνία:</strong>{" "}
                    {new Date(selectedAppointment?.date).toLocaleDateString("el-GR")}
                </p>
            </div>
            <div className="flex justify-center gap-6">
                <button
                    onClick={() => setStep("search")}
                    className="px-8 py-3 bg-[#9ca3af] text-white font-bold rounded-xl hover:bg-[#888f9b] cursor-pointer"
                >
                    Όχι, Επιστροφή
                </button>
                <button
                    onClick={handleCancelAppointment}
                    className="px-8 py-3 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 shadow-md cursor-pointer"
                >
                    Ναι, Ακύρωση
                </button>
            </div>
...
export default function ClientAppointments({ step, setStep }: Props) {
    ...
    const [searchFilters, setSearchFilters] = useState({
        date: "",
        location: "", // Removed time and specialty
    });
