                ...
                <table className="w-full min-w-[700px] border-collapse">
                    ...
                    <tbody className="text-sm">
                        {declarations.length === 0 ? (
                            <tr>
                                <td colSpan={5} className="text-center py-8 text-gray-500">Δεν βρέθηκαν δηλώσεις στο ιστορικό.</td>
                            </tr>
                        ) : (
                            declarations.map((item: any, idx: number) => {
                                const isFinal = item.status === "submitted"; 
                                const typeLabel = item.type === "lost" ? "Δήλωση Απώλειας" : "Δήλωση Εύρεσης";
                                const statusLabel = item.status === "saved" ? "Προσωρινή Αποθήκευση" : "Οριστική Υποβολή";
                                const statusColor = item.status === "saved" ? "bg-yellow-100 text-yellow-800" : "bg-green-100 text-green-800";

                                return (
                                    <tr key={item.id || idx} className="bg-[#f9f9f9] border-b-8 border-[#e5e5e5] last:border-0 rounded-xl">
                                        <td className="py-5 pl-4 rounded-l-xl font-medium text-gray-700 text-left">{typeLabel}</td>
                                        <td className="py-5 text-gray-600 text-center">{item.petName || "-"}</td>
                                            ...
                                            <div className="flex justify-end gap-3">
                                                <button 
                                                    ...
                                                    className={`p-2 rounded-lg transition-colors ${
                                                        isFinal ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-[#e0e0e0] hover:bg-[#d0d0d0] text-gray-700'
                                                    }`}
                                                >
...
const DeclarationForm = ({ type, formData, handleInputChange, handlePetSelection, handleImageUpload, pets, user, handleSave, errors, imagePreview }: any) => {
    return (
        <div className="bg-[#e5e5e5] rounded-3xl p-6 sm:p-12 shadow-lg max-w-5xl mx-auto w-full">
            <h1 className="text-3xl font-bold mb-3 text-[#303030] text-left">
                {formData.id ? "Επεξεργασία Δήλωσης" : (type === "lost" ? "Δήλωση απώλειας κατοικιδίου" : "Δήλωση εύρεσης κατοικιδίου")}
            </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                <div className="flex flex-col items-start w-full">
                    <label className="text-xs font-bold mb-2 ml-1 text-gray-600">Όνομα Κατοικιδίου {type==="lost" && "(επιλογή)"}</label>
                    {type === "lost" ? (
                        <select 
                        ...
                        >
                            ...
                            {pets.length > 0 ? (
                                ...
                                <option value="" disabled>Δεν βρέθηκαν κατοικίδια</option>
                            )}
                        </select>
                    ) : (
                        <input 
                ...
                <div className="flex flex-col items-start w-full">
                    ...
                    {errors.species && <span className="text-red-500 text-xs ml-1 mt-1">{errors.species}</span>}
                </div>
                <div className="flex flex-col items-start w-full">
                    ...
                    {errors.gender && <span className="text-red-500 text-xs ml-1 mt-1">{errors.gender}</span>}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-8 mb-8">
                <div className="flex flex-col items-start max-w-xl w-full">
                    ...
                    />
                    {errors.location && <span className="text-red-500 text-xs ml-1 mt-1">{errors.location}</span>}
                </div>
            </div>

            <div className="grid grid-cols-1 gap-8 mb-8">
                <div className="flex flex-col items-start max-w-xs w-full">
                    <label className="text-xs font-bold mb-2 ml-1 text-gray-600">Ημερομηνία *</label>
                    <input 
                        type="date" 
                        value={formData.date}
            ...
            <div className="mb-10 w-full">
                <label className="text-xs font-bold mb-2 ml-1 text-gray-600 block text-left">Ανεβάστε φωτογραφία του κατοικιδίου (Προαιρετικό)</label>
                <div className="flex items-center gap-4">
                    ...
                    <div className="relative w-full">
                        ...
                        <div className="w-full bg-white border border-dashed border-gray-400 rounded-xl p-12 flex items-center justify-center text-gray-500 text-sm hover:bg-gray-50 transition-colors">
                            {imagePreview ? "Αλλαγή φωτογραφίας" : "Επιλέξτε αρχεία ή σύρετε τα εδώ"}
                        </div>
                    </div>
...
export default function Declarations({ step, setStep }: DeclarationsProps) {
        ...
        const fetchData = async () => {
            try {
                ...
                if (Array.isArray(decData)) {
                    decData.sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime());
                }
    ...
    const handlePetSelection = (petId: string) => {
        ...
        if (selectedPet) {
            ...
            const mappedGender = (selectedPet.gender === "female" || selectedPet.gender === "Θηλυκό") ? "Θηλυκό" : "Αρσενικό";
    ...
    const handleSave = async (status: "submitted" | "saved") => {
        ...
        const confirmMessage = status === "submitted" 
        ...
        try {
            ...
            alert("Υπήρξε πρόβλημα κατά την αποθήκευση. Βεβαιωθείτε ότι το json-server τρέχει και το endpoint 'declarations' υπάρχει.");
        }
    };
